...
- `components/game/game-shell.tsx` no longer uses the shadcn `<Button>` primitive. Choice buttons render as outlined plain buttons themed with city CSS variables (translucent border, transparent background, accent text), matching the title-screen role-card pattern. Restart CTAs (Play again, Try again) render as filled buttons with the city accent color as background and city background color as text. Fixes the dark-teal-on-dark-teal contrast issue introduced when branding wiring landed.
